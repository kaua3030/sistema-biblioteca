...
exto completo (cole no README onde quiser)
## Testes e CI — instruções (conforme atividade)

### Requisitos mínimos
- Node.js (recomendado): 20.x
- npm (recomendado): 9.x / 11.x
- Docker & Docker Compose (para testes de integração com PostgreSQL)

---

### Rodar testes backend (integração com PostgreSQL)
1. Subir um Postgres de teste:
```bash
...
Definir variável de ambiente usada pelos testes:
Git Bash / WSL:
...
Se o CI usar npm ci e falhar por mismatch do package-lock.json, gere um lockfile limpo localmente:

rm -rf node_modules package-lock.json
npm install

e então commite o novo package-lock.json.

Comportamento esperado do CI
Instalar dependências (usar npm ci quando o lockfile estiver sincronizado).
Iniciar um serviço PostgreSQL temporário para testes de integração e exportar TEST_DATABASE_URL.
Executar npm test no backend e no frontend; parar o pipeline se qualquer teste falhar.
Só construir imagens/continuar pipeline se os testes passarem.
Checklist de entrega
 README atualizado com as instruções acima.
 Testes backend e frontend executam localmente conforme instruções.
 CI passa antes do merge para main.# (## Testes e CI — instruções (conforme atividade)
...
Checklist de entrega
 README atualizado com as instruções acima.
 Testes backend e frontend executam localmente conforme instruções.
 CI passa antes do merge para main.A)
